...
## ⚠️ DO NOT MODIFY (Auto-generated by Cron)

These files/directories are auto-generated by cron jobs. Do NOT edit or overwrite them:

- `scripts/daily-newsletter.sh` — pipeline entry point
- `scripts/write-newsletter.ts` — newsletter generation logic
- `scripts/collect-news.ts` — data collection
- `content/newsletters/` — all newsletter content (auto-generated and pushed by cron)

## Cron Schedule (Production)

All times shown in SGT (UTC+8). **Avoid git push during these windows to prevent merge conflicts.**

**⚠️ When changing any cron schedule, you MUST update ALL sources of truth — not just this table:**
- **VPS system crontab** (AI Dev, SEO Pipeline): `ssh loreai 'crontab -l'` / `ssh loreai '... | crontab -'`
- **OpenClaw cron** (AI Daily, AI Product, Indie, ai-newsletter-scout): edit `~/.openclaw/cron/jobs.json` on VPS via `ssh loreai`
- **This CLAUDE.md table** and danger windows below

| Job | SGT Time | UTC | Days | What |
|-----|----------|-----|------|------|
