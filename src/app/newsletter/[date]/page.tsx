...
async function getNewsletterData(date: string): Promise<DailyDigest | null> {
  try {
...
export default async function NewsletterDatePage({ params }: { params: Promise<{ date: string }> }) {
  const { date } = await params
  const digest = await getNewsletterData(date)

  if (!digest) {
    notFound()
  }

  const formattedDate = formatDateLong(digest.date)
