...
AI 搜索引擎（Perplexity、ChatGPT、Google AI Overview）会直接提取和引用内容。让文章更容易被引用：

### 数据摘要块
每篇文章必须包含至少一个格式清晰的数据块：

```markdown
**Claude Opus 4.6 关键数据：**
- SWE-bench Verified：72.5%（较上代提升 3.2%）
- 上下文窗口：1M tokens
- API 价格：$15/M 输入，$75/M 输出
- 发布时间：2026 年 2 月
```

### 引用友好格式规则
- 关键数据用**加粗 + 列表**呈现（不要埋在段落里）
- 每篇至少一个对比表格
- 每个小节开头写一句可以独立作为答案的陈述
- 避免模糊表达——务必包含具体数字、日期、模型名称
- 首次出现的术语明确定义（AI 引擎会提取定义）

### 避免
- ❌ "该模型表现出了显著的提升"（模糊，无法引用）
- ✅ "Claude Opus 4.6 在 SWE-bench 上得分 72.5%，较 Opus 4 提升 3.2%"（可引用）
